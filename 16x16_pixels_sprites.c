...
void main( void ) {
  ...
  Screen(1);
  SpriteReset();
  SpriteDouble(); // Set double sprite size
  
  // The 16 x 16 pixels Sprite is made of 4 patterns

  SetSpritePattern( 0, point_pattern0,8 ); // pattern number, pattern bytes, pattern lines
  ...
  {  
      ...
      EnableInterupt()
      Halt();
      PutSprite (1,0,x,y,8); // sprite layer, sprite pattern, x, y, color
